...
def login():
    ...
    3. The user database object model is stored in a Python data structure and the controller makes it available to the Flask-Login method `login_user()` by passing it as a parameter to that method.
        ...
        d. Successfully calling `login_user()` creates a session, as all subsequent requests will now have access to user's database object model. Without using sessions, the user would have to send authenticated requests each time they wanted to access a protected view instead of just once at log in.
    
    4. The user is redirected to the `index` view after successfully logging in.
